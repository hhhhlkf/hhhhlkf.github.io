...
(function (exports) {
    document.write("<script language=javascript src='/mersenne-twister.js'></script>");
    var _this = exports;
    var E = Math.E,
        PI = Math.PI,
        PI_2 = PI / 2;
    ...
    exports.uniform = function (min, max) {
        return min + (max - min) * _this.newrandom();
    };
    exports.variance = function (nf) {
        let sum = 0;
        s = 0;
        for (let i = 1; i <= nf; i++) {
            sum += i;
        ...
        return Math.sqrt((s / nf), 2);
    }
    exports.primNum = function (nf) {
        n = nf;
        u = nf / 2;
        t = _this.variance(nf);
    };
    exports.newrandom = function () {
        let total = 0.0;
        let generator = new MersenneTwister(Date.now() + u + t);
        let r = generator.random_long();
        r = r * 100000;
        for (let i = 0; i < n; i++) {
            // console.log(1111);
            let base = 256.0;
            ...
            let b = 139.0;
            let temp1 = a * (r) + b;
            let temp2 = floor((temp1 / base));
            let temp3 = temp1 - temp2 * base;
            r = temp3 / base;
            total = total + r;
        }
        return (total - n / 2) / sqrt(n / 12)-Math.floor((total - n / 2) / sqrt(n / 12));
    };
